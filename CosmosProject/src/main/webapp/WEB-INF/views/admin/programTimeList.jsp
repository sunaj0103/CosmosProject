	  	...
	  	<div class="admin">
			...
			<ul id="programTimeList">
				...
				<c:forEach var="vo" items="${pd_list}">
					<li class="programList2"><a href="/web/admin/programTimeListView?num=${vo.pd_num}">${vo.title}</a></li>
					...
					<c:forEach var="p" begin="${pageVo.startPageNum }" end="${pageVo.startPageNum+pageVo.onePageCount-1}">
						<c:if test="${p<=pageVo.totalPageCount}">
							<c:if test="${pageVo.currentPage==p }">
								<li class="active"><a href="/web/admin/programTimeList?currentPage=${p}">${p}</a></li>
							</c:if>
							<c:if test="${pageVo.currentPage!=p }">
								<li><a href="/web/admin/programTimeList?currentPage=${p}">${p}</a></li>
							</c:if>
						</c:if>
					</c:forEach>
